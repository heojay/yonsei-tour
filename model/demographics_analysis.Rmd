...
여행지 추천에서 당연히 방문 목적은 당연히 고려를 해야된다.

하지만 인구 통계정보를 고려하는 것이 맞을까? 라는 질문에서 시작한다.
...
방문 목적을 공변량으로 둔 다음에 선형회귀를 진행해보자.
...
방문목적, 국적을 공변량으로 둔 다음에 선형회귀를 진행해보자.
...
성별은 담기지 않는다. 성별을 아는 것은 유의한 정보일 수 있으나 
...
```{r}
ggplot(estimated_df, aes(x=topic2, color='black')) + stat_density(geom='line') +
    geom_density(data=df, aes(color='red')) +
    scale_colour_manual(name = 'Type',
                        values =c('black'='black','red'='red'), labels = c('예측','실제'))+
    labs(title='예측토픽과 실제토픽') +
...
그리고 검은색(estimated topic)에서 빨간색(actual value)으로 가는 적절한 변환을 못찾겠다.
...
**실제 방문지 토픽** VS **Estimated Topic** | 방문목적, 인구통계 정보
...
Given (40 - 1) 범주로 묶은 관광지 리뷰로 학습한 LDA를 이용하여, {162개의 관광지}의 토픽을 뽑은 상황이다.
